...
# 定义一个函数来将 test_mode 转换为对应的列表
def parse_test_mode(mode_str):
    if mode_str == 'v2i':
        return [2, 1]  # VIS to IR
    elif mode_str == 'i2v':
        return [1, 2]  # IR to VIS
    else:
        raise ValueError(f"Unknown test_mode: {mode_str}. Use 'v2i' or 'i2v'.")

parser = argparse.ArgumentParser(description='PyTorch Cross-Modality Training')
parser.add_argument('--dataset', default='sysu', help='dataset name: regdb or sysu]')
parser.add_argument('--data_path', default='sysu', help='dataset path')
parser.add_argument('--lr', default=0.1, type=float, help='learning rate, 0.00035 for adam')
parser.add_argument('--optim', default='sgd', type=str, help='optimizer')
parser.add_argument('--arch', default='resnet50', type=str, help='network baseline:resnet18 or resnet50')
parser.add_argument('--resume', '-r', default='', type=str, help='resume from checkpoint')
parser.add_argument('--test-only', action='store_true', help='test only')
parser.add_argument('--model_path', default='/root/autodl-tmp/deen_reid/synthetic_reid_sysu/sysu_cam7_12_1011/', type=str, help='model save path')
...
if dataset == 'sysu':
    ...
    test_mode = parse_test_mode(args.test_mode)
    pool_dim = 2048
